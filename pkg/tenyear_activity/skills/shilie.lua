...
Fk:loadTranslationTable{
  ...
  [':shilie'] = '出牌阶段限一次，你可以选择一项：1.回复1点体力，然后将两张牌置为“示烈”牌（不足则全放，总数不能大于游戏人数）；2.失去1点体力，然后获得两张“示烈”牌。<br>你死亡时，你可将“示烈”牌交给除伤害来源外的一名其他角色。',
  ['$shilie1'] = '荆州七郡，亦有怀义之人！',
...
shilie:addEffect('active', {
  ...
  can_use = function(self, player)
    return player:usedSkillTimes(shilie.name, Player.HistoryPhase) == 0
  end,
  card_filter = Util.FalseFunc,
  on_use = function(self, room, effect)
    local player = room:getPlayerById(effect.from)
    if skill.interaction.data == "recover" then
      ...
      if not player:isNude() then
        ...
        if n > 0 then
          ...
          room:moveCardTo(to_remove, Card.DiscardPile, nil, fk.ReasonPutIntoDiscardPile, shilie.name, nil, true, player.id)
        end
      end
    else
      ...
      local cards = room:askToCards(player, {
        min_num = math.min(#player:getPile("$shilie"), 2),
        max_num = 2,
        pattern = ".|.|.|$shilie",
        prompt = "#shilie-get",
        skill_name = shilie.name,
      ...
      room:moveCardTo(cards, Card.PlayerHand, player, fk.ReasonJustMove, shilie.name, nil, false, player.id)
    end
  end,
})
...
shilie:addEffect(fk.Death, {
  ...
  can_trigger = function(self, event, target, player, data)
    return target == player and player:hasSkill(shilie.name, false, true) and #player:getPile("$shilie") > 0
  end,
