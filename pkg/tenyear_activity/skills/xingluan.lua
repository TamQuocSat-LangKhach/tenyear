...
xingluan:addEffect(fk.CardUseFinished, {
  ...
  can_trigger = function(self, event, target, player, data)
    return target == player and player:hasSkill(xingluan.name) and player.phase == Player.Play
      and #TargetGroup:getRealTargets(data.tos) == 1 and player:usedSkillTimes(xingluan.name, Player.HistoryPhase) == 0
  end,
