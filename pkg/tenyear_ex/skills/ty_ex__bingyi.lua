local ty_ex__bingyi = fk.CreateSkill {
  name = "ty_ex__bingyi"
}
...
ty_ex__bingyi:addEffect(fk.EventPhaseStart, {
  anim_type = "drawcard",
  can_trigger = function(self, event, target, player, data)
    return target == player and player:hasSkill(ty_ex__bingyi.name) and player.phase == Player.Finish and not player:isKongcheng()
  end,
  on_use = function(self, event, target, player, data)
    ...
    if table.every(cards, function(id) return Fk:getCardById(id).color == Fk:getCardById(cards[1]).color end) then
      local tos = room:askToChoosePlayers(player, {
        ...
        skill_name = ty_ex__bingyi.name
      })
      if #tos > 0 then
        for _, p in ipairs(tos) do
          room:getPlayerById(p):drawCards(1, ty_ex__bingyi.name)
        end
        if table.every(cards, function(id) return Fk:getCardById(id).number == Fk:getCardById(cards[1]).number end) then
          player:drawCards(1, ty_ex__bingyi.name)
        end
      end
    end
  end,
