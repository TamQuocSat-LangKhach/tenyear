...
local fuman = fk.CreateActiveSkill{
  ...
  target_filter = function(self, to_select, selected)
    return #selected == 0 and not table.contains(Self:getTableMark("ty_ex__fuman-phase"), to_select)
  end,
  on_use = function(self, room, effect)
    local player = room:getPlayerById(effect.from)
    ...
    if #card > 0 then
      ...
      room:moveCardTo(card, Card.PlayerHand, target, fk.ReasonJustMove, self.name, nil, true, player.id, {"@@ty_ex__fuman", player.id})
    end
  end,
}
local fuman_delay = fk.CreateTriggerSkill{
  ...
  on_cost = Util.TrueFunc,
  on_use = function (self, event, target, player, data)
    player:drawCards(data.extra_data.ty_ex__fuman_draw[player.id], self.name)
  end,

  refresh_events = {fk.AfterTurnEnd, fk.AfterCardsMove},
  can_refresh = function (self, event, target, player, data)
