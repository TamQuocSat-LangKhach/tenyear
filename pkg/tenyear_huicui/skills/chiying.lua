...
Fk:loadTranslationTable{
  ...
  [':chiying'] = '出牌阶段限一次，你可以选择一名体力值不大于你的角色，其可以弃置其攻击范围内任意名其他角色各一张牌。若选择的角色不为你，其获得其中的基本牌。',
...
chiying:addEffect('active', {
  ...
  target_filter = function(self, player, to_select, selected)
    return #selected == 0 and Fk:currentRoom():getPlayerById(to_select).hp <= player.hp
  end,
  on_use = function(self, room, effect)
    ...
    room:moveCardTo(ids, Card.PlayerHand, target, fk.ReasonPrey, chiying.name, nil, true, target.id)
  end,
})
