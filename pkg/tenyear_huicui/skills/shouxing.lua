...
shouxing:addEffect('prohibit', {
  is_prohibited = function(self, from, to, card)
    return table.contains(card.skillNames, shouxing.name) and (from:distanceTo(to) ~= #card.subcards or from:inMyAttackRange(to))
  end,
})

shouxing:addEffect('targetmod', {
  bypass_distances = function(self, player, skillName, card)
    return skillName.trueName == "slash_skill" and card and table.contains(card.skillNames, shouxing.name)
  end,
  bypass_times = function(self, player, skillName, scope, card)
    return skillName.trueName == "slash_skill" and scope == Player.HistoryPhase and card and table.contains(card.skillNames, shouxing.name)
  end,
})
