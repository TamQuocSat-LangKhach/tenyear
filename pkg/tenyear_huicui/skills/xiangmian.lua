...
Fk:loadTranslationTable{
  ["xiangmian"] = "相面",
  [":xiangmian"] = "出牌阶段限一次，你可以令一名其他角色进行一次判定，当该角色使用判定花色的牌或使用第X张牌后（X为判定点数），"..
  "其失去所有体力。每名其他角色限一次。",

  ["#xiangmian"] = "相面：令一名其他角色判定",
  ["@xiangmian"] = "相面",
...
xiangmian:addEffect("active", {
  ...
  can_use = function(self, player)
    return player:usedSkillTimes(xiangmian.name, Player.HistoryPhase) == 0
  end,
  card_filter = Util.FalseFunc,
  target_filter = function(self, player, to_select, selected)
    return #selected == 0 and to_select ~= player and to_select:getMark("xiangmian_suit") == 0
  end,
  target_num = 1,
  on_use = function(self, room, effect)
    ...
    room:setPlayerMark(target, "@xiangmian", string.format("%s%d",
      Fk:translate(target:getMark("xiangmian_suit")), target:getMark("xiangmian_num")))
  end,
})
