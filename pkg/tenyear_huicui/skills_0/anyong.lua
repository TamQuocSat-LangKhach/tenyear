...
anyong:addEffect(fk.Damage, {
  ...
  can_trigger = function(self, event, target, player, data)
    if player:hasSkill(anyong) and not player:isNude() and target and target == player.room.current and not data.to.dead and data.damage == 1 then
  ...
  on_use = function(self, event, target, player, data)
    local room = player.room
    room:throwCard(event:getCostData(self), anyong.name, player, player)
