...
Fk:loadTranslationTable{
  ...
  ['@@konghou-inhand'] = '箜篌',
  [':xianshu'] = '出牌阶段，你可以将一张“箜篌”牌交给一名其他角色并摸X张牌（X为你与该角色体力值之差且至多为5），若此牌为：红色，且该角色体力值不大于你，该角色回复1点体力；黑色，且该角色体力值不小于你，该角色失去1点体力。',
...
xianshu:addEffect('active', {
  ...
  on_use = function(self, room, effect)
    ...
    if x > 0 then
      room:drawCards(player, math.min(x, 5), { skill_name = xianshu.name })
    end
    if player.dead or target.dead then return end
    ...
    if color == Card.Black and target.hp >= player.hp then
      room:loseHp(target, 1, { skill_name = xianshu.name })
    end
  end,
})
