...
yingshui:addEffect('active', {
  ...
  on_use = function(self, room, effect, event)
    ...
    room:moveCardTo(Fk:getCardById(effect.cards[1]), Card.PlayerHand, target, fk.ReasonGive, yingshui.name, nil, false, player.id)
    ...
    if player.dead or #target:getCardIds("he") < 2 then
      ...
      if #cards > 1 then
        room:moveCardTo(cards, Card.PlayerHand, player, fk.ReasonGive, yingshui.name, nil, true, target.id)
      else
        room:damage{
          from = player,
          to = target,
