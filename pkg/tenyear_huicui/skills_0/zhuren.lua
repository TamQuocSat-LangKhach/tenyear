...
Fk:loadTranslationTable{
  ['zhuren'] = '铸刃',
  [':zhuren'] = '出牌阶段限一次，你可以弃置一张手牌。根据此牌的花色点数，你有一定概率打造成功并获得一张武器牌（若打造失败或武器已有则改为摸一张【杀】，花色决定武器名称，点数决定成功率）。此武器牌进入弃牌堆时，将之移出游戏。',
  ['$zhuren1'] = '造刀三千口，用法各不同。',
  ['$zhuren2'] = '此刀，可劈铁珠之筒。',
}
...
zhuren:addEffect('active', {
  ...
  can_use = function(self, player)
    return player:usedSkillTimes(zhuren.name, Player.HistoryPhase) == 0 and not player:isKongcheng()
  end,
  ...
  on_use = function(self, room, effect)
    ...
    if name == "slash" then
      ...
      if #ids > 0 then
        room:moveCardTo(ids, Card.PlayerHand, player, fk.ReasonJustMove, zhuren.name, nil, true, player.id)
      end
    elseif get then
      room:setCardMark(Fk:getCardById(get), MarkEnum.DestructIntoDiscard, 1)
      room:moveCardTo(get, Card.PlayerHand, player, fk.ReasonJustMove, zhuren.name, nil, true, player.id)
    end
  end,
})
