...
Fk:loadTranslationTable{
  ...
  [':gaojian'] = '当你于出牌阶段使用锦囊牌结算完毕进入弃牌堆时，你可以选择一名其他角色，其依次展示牌堆顶的牌直到出现锦囊牌（至多五张），然后其选择一项：1.使用此牌；2.将任意张手牌与等量展示牌交换。然后将剩余牌置于牌堆顶。',
...
gaojian:addEffect(fk.CardUseFinished, {
  ...
  can_trigger = function(self, event, target, player, data)
    return target == player and player:hasSkill(gaojian.name) and data.card:isCommonTrick() and player.phase == Player.Play and
  ...
  on_use = function(self, event, target, player, data)
    ...
    for i = 1, 5, 1 do
      ...
      room:moveCardTo(card, Card.Processing, nil, fk.ReasonJustMove, gaojian.name, nil, true, to.id)
    ...
    if Fk:getCardById(cards[#cards]).type == Card.TypeTrick then
      ...
      if room:askToUseRealCard(to, {
        pattern = {card},
        skill_name = gaojian.name,
        prompt = "#gaojian-use:::"..Fk:getCardById(card):toLogString(),
        expand_pile = {card},
        bypass_times = true,
        extra_data = {
          extraUse = true,
    ...
    if not yes then
      local results = room:askToArrangeCards(to, {
        skill_name = gaojian.name,
        card_map = {cards, to:getCardIds("h")},
        prompt = "#gaojian-exchange",
        area_names = {"hand_card"},
      })
      if #results > 0 then
        U.swapCardsWithPile(to, results[1], results[2], gaojian.name, "Top")
      end
    end
    ...
    if #cards > 0 then
      ...
      room:moveCards({
        ids = cards,
        to_area = Card.DrawPile,
        move_reason = fk.ReasonPut,
        skill_name = gaojian.name,
      })
    end
  end,
