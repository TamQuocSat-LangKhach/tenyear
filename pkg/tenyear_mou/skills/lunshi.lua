...
lunshi:addEffect("viewas", {
  ...
  enabled_at_response = function (self, player, response)
    if response or player:getMark("tymou__lunshi_activated") == 0 or player:isKongcheng() then return end
    local red, black = 0, 0
...
lunshi:addEffect(fk.HandleAskForPlayCard, {
  can_refresh = function(self, event, target, player, data)
    if data.afterRequest and (data.extra_data or {}).lunshiEffected then
      ...
      return player:hasSkill(lunshi.name) and data.eventData and
        data.eventData.from and data.eventData.from ~= player.id and
        data.eventData.to and data.eventData.to ~= data.eventData.from and
