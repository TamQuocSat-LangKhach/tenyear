...
qijing:addEffect(fk.EventTurnChanging, {
  can_refresh = function(self, event, target, player, data)
    return target == player and player.room:getBanner("qijing_using") ~= nil
  end,
  on_refresh = function(self, event, target, player, data)
    local room = player.room
    local index = room:getBanner("qijing_using")
    if index == -1 then
    ...
    room:setBanner("qijing_using", nil)
  end,
})
