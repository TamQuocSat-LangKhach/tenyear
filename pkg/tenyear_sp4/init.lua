...
local ty__zhennan = fk.CreateTriggerSkill{
  ...
  on_cost = function(self, event, target, player, data)
    local to = player.room:askForChoosePlayers(player, table.map(player.room:getOtherPlayers(player, false), Util.IdMapper), 1, 1, "#ty__zhennan-choose", self.name, true)
    if #to > 0 then
      self.cost_data = to[1]
      return true
    end
...
local peiniang_trigger = fk.CreateTriggerSkill{
  ...
  on_cost = function (self, event, target, player, data)
    ...
    local card = room:askForCard(player, 1, 1, true, "peiniang", true, tostring(Exppattern{ id = ids }),
      "#peiniang-use::"..target.id)
    if #card > 0 then
      room:doIndicate(player.id, { target.id })
...
Fk:loadTranslationTable{
  ...
  [":peiniang"] = "你可以将“异瞳”花色的牌当【酒】使用（不计次数）。"..
    "当一名角色进入濒死状态时，你可以将一张【酒】或“异瞳”花色的牌当【酒】（使用方法②）对其使用。",
