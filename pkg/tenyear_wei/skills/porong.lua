...
porong:addEffect(fk.CardUsing, {
  can_trigger = function(self, event, target, player, data)
    return target == player and player:hasSkill(porong) and data.card.trueName == "slash" and
  ...
  on_use = function (self, event, target, player, data)
    ...
    for _, id in ipairs(targets) do
      ...
      if not p:isKongcheng() then
        ...
        room:moveCardTo(card, Card.PlayerHand, player, fk.ReasonPrey, porong.name, nil, false, player.id)
      end
    end
  end,
})
...
porong:addEffect(fk.AfterCardUseDeclared, {
  can_trigger = function (self, event, target, player, data)
    return target == player and player:hasSkill(porong, true)
  end,
  on_use = function (self, event, target, player, data)
    local room = player.room
