...
sanwen:addEffect(fk.AfterCardsMove, {
  ...
  can_trigger = function(self, event, target, player, data)
    if player:hasSkill(sanwen.name) and player:usedSkillTimes(sanwen.name, Player.HistoryTurn) < 1 then
      local get = {}
      ...
      if #throw > 0 then
        event:setCostData(self, {throw, show})
        return true
      end
    end
  end,
  on_cost = function (self, event, target, player, data)
    ...
    return player.room:askToSkillInvoke(player, {
      ...
      prompt = "#sanwen-invoke:::"..num
    })
  end,
  on_use = function(self, event, target, player, data)
    local room = player.room
    local throw = event:getCostData(self)[1]
    local show = event:getCostData(self)[2]
