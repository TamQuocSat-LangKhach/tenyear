local zhenfengf = fk.CreateSkill {
  name = "zhenfengf"
}
...
Fk:loadTranslationTable{
  ...
  ['@zhenfengf'] = '镇锋',
  [':zhenfengf'] = '每回合限一次，一名其他角色于其回合内使用牌时，若其手牌数不大于体力值，你可以猜测其手牌中与此牌类别相同的牌数。若你猜对，你摸X张牌并视为对其使用一张【杀】（X为你连续猜对次数且最多为5）；若猜错且差值大于1，则视为其对你使用一张【杀】。',
...
zhenfengf:addEffect(fk.CardUsing, {
  ...
  on_use = function(self, event, target, player, data)
    ...
    if tonumber(choice) == n then
      ...
      player:drawCards(math.min(player:getMark("@zhenfengf"), 5), zhenfengf.name)
      if not player.dead and not target.dead then
        room:useVirtualCard("slash", nil, player, target, zhenfengf.name, true)
      end
    else
      room:setPlayerMark(player, "@zhenfengf", 0)
      if math.abs(tonumber(choice) - n) > 1 then
        room:useVirtualCard("slash", nil, target, player, zhenfengf.name, true)
      end
    end
  end,
})
