...
Fk:loadTranslationTable{
  ...
  [':ty__zhuijix'] = '当你死亡后，你可以令一名角色从牌堆和弃牌堆中随机使用有空余栏位的装备牌，直至其装备区满，若如此做，当其失去以此法使用的装备牌后，废除对应的装备栏。',
      ...
      while #subtypes > 0 do
        ...
        if to:hasEmptyEquipSlot(subtype) then
          ...
          if #cards > 0 then
            ...
            room:addTableMark(to, skill.name, card)
            room:useCard{
              from = to.id,
        ...
        if move.from == player.id then
          for _, info in ipairs(move.moveInfo) do
            if info.fromArea == Card.PlayerEquip and table.contains(player:getTableMark("ty__zhuijix"), info.cardId) and
              ...
              if e and e.data[3] == skill.name then  --FIXME：防止顶替装备时重复触发
                return false
              end
              return true
            end
