...
local ty_ex__jiezhong = fk.CreateTriggerSkill{
  ...
  can_trigger = function(self, event, target, player, data)
    return target == player and player:hasSkill(self.name) and player.phase == Player.Play and math.min(player.maxHp, 5) > #player.player_cards[Player.Hand] and player:usedSkillTimes(self.name, Player.HistoryGame) == 0
  end,
  on_cost = function(self, event, target, player, data)
     local draw = math.min(player.maxHp, 5) - #player.player_cards[Player.Hand]
    return player.room:askForSkillInvoke(player, self.name, nil, "#jiezhong-invoke:::"..draw)
  end,
  on_use = function(self, event, target, player, data)
    local room = player.room
    local n = math.min(player.maxHp, 5) - #player.player_cards[Player.Hand]
...
Fk:loadTranslationTable{
  ["ty_ex__caozhi"] = "界曹植",
  ["ty_ex__jiushi"] = "酒诗",
  [":ty_ex__jiushi"] = "①若你的武将牌正面朝上，你可以（在合理的时机）将你的武将牌翻面来视为使用一张【酒】。②当你的武将牌背面朝上时你受到伤害，你可在伤害结算后将之翻回正面。③当你使用酒时，你令你使用杀的次数上限+1，直到你的下个回合结束。",
...
Fk:loadTranslationTable{
  ["ty_ex__chengong"] = "陈宫",
  ["ty_ex__mingce"] = "明策",
  [":ty_ex__mingce"] = "出牌阶段限一次，你可以交给一名其他角色一张装备牌或【杀】，其选择一项：1.视为对其攻击范围内的另一名由你指定的角色使用一张【杀】，若此杀造成伤害则执行选项2；2.你与其各摸一张牌。",
...
local ty_ex__zili = fk.CreateTriggerSkill{
  ...
  on_use = function(self, event, target, player, data)
    ...
    room:handleAddLoseSkills(player, "ty_ex__paiyi", nil)
  end,
}
    ...
    for i = 1, 2, 1 do
     ...
     if #targetsx > 0 then
      ...
      if #tos == 0 then
       ...
       local daren = room:getPlayerById(tos[1])
       room:damage{
          from = player,
          to = daren,
          damage = 1,
...
local ex__yongjin = fk.CreateActiveSkill{
  ...
  frequency = Skill.Limited,
  can_use = function(self, player)
    return player:usedSkillTimes(self.name, Player.HistoryGame) == 0 and table.find(Fk:currentRoom().alive_players, function (p) return #p.player_cards[Player.Equip] > 0 end)
  end,
  on_use = function(self, room, effect)
    local from = room:getPlayerById(effect.from)
   for i = 1, 3, 1 do
     if table.find(room.alive_players, function (p) return #p.player_cards[Player.Equip] > 0 end) then
          local to = room:askForChooseToMoveCardInBoard(from, "#ex__yongjin-choose", self.name, true, "e")
        if #to == 2 then
          room:askForMoveCardInBoard(from, room:getPlayerById(to[1]), room:getPlayerById(to[2]), self.name, "e")
        end
     end
   end
  end,
...
Fk:loadTranslationTable{
  ["ty_ex__lingtong"] = "界凌统",
  ["ty_ex__xuanfeng"] = "旋风",
  [":ty_ex__xuanfeng"] = "当你失去装备区里的牌，或于弃牌阶段弃掉两张或更多的牌时，你可以依次弃置一至两名角色的共计两张牌。;若此时是你的回合内，则你可以对其中一名角色造成一点伤害。",
