...
Fk:loadTranslationTable{
  ...
  ["#shouli_trigger"] = "狩骊",
  ["#shouli_delay"] = "狩骊",
  [":shouli"] = "游戏开始时，从下家开始所有角色随机使用牌堆中的一张坐骑。你可将场上的一张进攻马当【杀】（不计入次数，有次数限制）、"..
  "防御马当【闪】使用或打出，以此法失去坐骑的其他角色本回合非锁定技失效，你与其本回合受到的伤害+1且改为雷电伤害（不叠加）。",
  ["hengwu"] = "横骛",
...
local sijun = fk.CreateTriggerSkill{
  ...
  on_use = function(self, event, target, player, data)
    ...
    while total > 0 and i < 999 do
      ...
      if id ~= nil and not table.contains(cards, id) then
