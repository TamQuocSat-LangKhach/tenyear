...
local yuxian = fk.CreateTriggerSkill{
  ...
  can_trigger = function(self, event, target, player, data)
    ...
    if player == target then
      return player.phase == Player.Play and #player:getTableMark("@[suits]yuxian") < 4 and
        data.card.suit ~= Card.NoSuit and U.IsUsingHandcard(player, data)
    else
      local room = player.room
      if room.current ~= target or data.card.suit == Card.NoSuit then return false end
      local mark = player:getTableMark("@[suits]yuxian")
      if #mark == 0 then return false end
      local use_e = room.logic:getCurrentEvent():findParent(GameEvent.UseCard, true)
      local turn_e = use_e:findParent(GameEvent.Turn, true)
      local events = room.logic:getEventsByRule(GameEvent.UseCard, #mark + 1, function(e)
        if e.id > use_e.id then return false end
        return e.data[1].from == target.id
      end, turn_e.id)
      return data.card.suit == mark[#events]
    end
  end,
  on_cost = function(self, event, target, player, data)
    if player == target then
      self.cost_data = {}
      return true
    elseif player.room:askForSkillInvoke(player, self.name, nil, "#yuxian-invoke::"..target.id) then
  ...
  on_use = function(self, event, target, player, data)
    if player == target then
      player.room:addTableMark(player, "@[suits]yuxian", data.card.suit)
    else
      player:drawCards(1, self.name)
...
local minshan = fk.CreateTriggerSkill{
  ...
  events = {fk.Damaged},
  can_trigger = function(self, event, target, player, data)
    return target == player and player:hasSkill(self) and data.card and data.card.suit ~= Card.NoSuit
  end,
  on_cost = function(self, event, target, player, data)
    local room = player.room
    local to = room:askForChoosePlayers(player, table.map(room.alive_players, Util.IdMapper),
    1, 1, "#minshan-choose:::"..data.card:getSuitString(true), self.name, true)
    if #to > 0 then
      self.cost_data = { tos = to }
      return true
    end
  ...
  on_use = function(self, event, target, player, data)
    local room = player.room
    local cards = room:getCardsFromPileByRule(".|.|" .. data.card:getSuitString(), 2)
    if #cards > 0 then
      room:obtainCard(self.cost_data.tos[1], cards, false, fk.ReasonJustMove, player.id, self.name)
    end
  end,
}
...
local xianniang = fk.CreateTriggerSkill{
  ...
  can_trigger = function(self, event, target, player, data)
    ...
    if event == fk.AfterCardsMove then
      if player:getMark("xianniang1-round") > 0 then return false end
      for _, move in ipairs(data) do
        if move.from == player.id and move.moveReason == fk.ReasonDiscard and move.proposer and move.proposer ~= player.id then
          for _, info in ipairs(move.moveInfo) do
            if info.fromArea == Card.PlayerHand or info.fromArea == Card.PlayerEquip then
              local x = player:getHandcardNum()
              return #player:getCardIds("e") > 0 or not table.every(player.room.alive_players, function (p)
                return p == player or p:getHandcardNum() < x or #p:getCardIds("he") == 0
              end)
            end
          end
        end
      ...
      if player == target and player:getMark("xianniang2-round") == 0 then
        local x = player:getHandcardNum()
        return (#player:getCardIds("e") > 0 or not table.every(player.room.alive_players, function (p)
          return p == player or p:getHandcardNum() < player:getHandcardNum() or #p:getCardIds("he") == 0
        end))
      end
    end
  end,
  on_cost = function(self, event, target, player, data)
    ...
    local to = room:askForChoosePlayers(player, table.map(table.filter(room.alive_players, function(p)
      if p == player then
        ...
        return p:getHandcardNum() >= x and #p:getCardIds("he") > 0
      end
    end), Util.IdMapper), 1, 1, "#xianniang-choose", self.name, true)
    if #to > 0 then
      self.cost_data = { tos = to }
      return true
    end
  ...
  on_use = function(self, event, target, player, data)
    local room = player.room
    if event == fk.AfterCardsMove then
      room:setPlayerMark(player, "xianniang1-round", 1)
    else
      room:setPlayerMark(player, "xianniang2-round", 1)
    end
    local to = room:getPlayerById(self.cost_data.tos[1])
    local ids = room:askForCardsChosen(player, to, 1, 2, (player == to) and "e" or "he", self.name)
    local x = #ids
    ...
    xianniangMove(ids, to.id, player.id, fk.ReasonPrey)
    ...
    if #tos > 0 then
      tos, ids = room:askForChooseCardsAndPlayers(player, 1, x, table.map(tos, Util.IdMapper),
      1, 1, ".", "#xianniang-give:::" .. x, self.name, true, true)
      if #tos > 0 and #ids > 0 then
        xianniangMove(ids, player.id, tos[1], fk.ReasonGive)
        if player.dead then return false end
      end
    end
  ...
  on_lose = function (self, player)
    local room = player.room
    room:setPlayerMark(player, "xianniang1-round", 0)
    room:setPlayerMark(player, "xianniang2-round", 0)
    room:setPlayerMark(player, "xianniang_num-round", 0)
  end,
}

local xianniang_filter = fk.CreateFilterSkill{
  name = "#xianniang_filter",
  mute = true,
  card_filter = function(self, card, player, isJudgeEvent)
    return card:getMark("@@xianniang-inhand") > 0 and table.contains(player.player_cards[Player.Hand], card.id)
  end,
  view_as = function(self, card)
...
xianniang:addRelatedSkill(xianniang_filter)
houcheng:addSkill(xianniang)

Fk:loadTranslationTable{
  ["xianniang"] = "献酿",
  [":xianniang"] = "每轮各限一次，当你因被其他角色弃置而失去牌后，或当你受到伤害后，你可以获得一名手牌数不小于你的角色的至多两张牌，"..
  "然后可以将至多等量的牌交给另一名角色，以此法获得及交给的牌中的基本牌在这些角色的手牌区里视为【酒】，"..
  "若你本轮内以此法获得过的牌数大于2，你失去1点体力。",
